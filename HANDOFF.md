...
## Update 2026-05-24 (s-итерация, вечер) — каскад UI/UX правок

После деплоя r-итерации в течение вечера навалилось много мелких задач от владельца. Все задеплоены, все закоммичены, прод HEAD = `8ca7d05`.

**s1. Selection-форма / Installation — модальное окно успеха.**
Раньше HTMX подменял только `<div id="selection-result">` ПОД формой — пользователь видел подтверждение и продолжавшую висеть форму. Теперь: `hx-target="body" hx-swap="beforeend"`, view возвращает `SUCCESS_MODAL_HTML` (overlay `fixed inset-0` с зелёным ✓, текстом «Заявка принята! Менеджер свяжется с вами в течение часа», кнопкой «На главную» и крестиком). Форма очищается через `this.reset()`. quick-order модалка на product_detail оставлена с inline success + auto-close через 2.5с.

**s2. Кнопка «Поделиться» в карточке товара (product_card.html).**
В правом верхнем углу каждой карточки — иконка share. Web Share API на mobile (родное системное окно), `navigator.clipboard.writeText` + toast «Ссылка скопирована» на desktop. Глобальные `window.shareProduct` и `window.showToast` в `base.html` — переиспользуемы.

**s3. Default «только Крым» в каталоге, главной и similar.**
Раньше: фильтр Симферополь был opt-in через чекбокс «Только в наличии». Теперь по умолчанию `catalog()` показывает **только то, что физически на крымском складе**. Opt-out через `?with_order=1` (чекбокс в сайдбаре «Показать также «под заказ»», ИЛИ кнопка-toggle в шапке каталога — `.catalog-toggle-btn`). Аналогично `home()` featured (всегда Крым+qty>0) и `product_detail()` similar (Крым с fallback на любые наличия). Удалён filter `in_stock` (стал дефолтом).

**s4. Merge полупромышленных категорий + exclude аксессуаров.**
Миграция `0012_merge_polyprom_and_disable_accessories`:
- 470 товаров из дубликата id=115 «Полупромышленные кондиционеры» перенесены в id=6 «Полупромышленные сплит-системы».
- У id=116 «Аксессуары для кондиционеров» выключен `sync_enabled`.
- Идемпотентна.

Code guard — `NON_RETAIL_Q` в `apps/catalog/filters.py`: regex по Product.title для исключения аксессуаров. Применяется в `catalog()`, `home()`, `product_detail.similar`, `quiz_logic._base_qs`. После s-итерации regex расширен на основе реальной выдачи прода (Риммер, Труборез, Сифон, Виброопоры, Коллектор, Манометр, Шланг, Адаптер «оконный», Чистящ средства и т.д.); сам regex теперь `(?:^|\s)(...)` — ловит слово в начале title ИЛИ после пробела.

Активные категории на проде: **3 шт** — Бытовые сплит-системы (2031 в каталоге / 277 в Крыму), Полупромышленные (489 / 8), Мобильные (96 / 12). Чистый каталог без кронштейнов/чехлов/инструментов.

**s5. Header — контуры nav-btn + Mobile.**
Все 5 ссылок (Каталог/Подбор/Монтаж/Доставка/Контакты) — pill-кнопки `.nav-btn` с border, hover → акцент-цвет + лифт. Стиль в `static/css/splithub.css`. Mobile drawer переделан в `grid grid-cols-2` тех же `.nav-btn`.

**s6. Catalog sidebar — подкатегории через URL-фильтр.**
Категории-кнопки `.cat-btn` (border 1.5px, активная подсвечена синим, счётчик в круглой pill-плашке). Под каждой — sub-list со ссылками подкатегорий:
- Бытовые → Инверторные / Классические (on-off) — через `?inverter=inverter|onoff`
- Полупромышленные → Кассетные / Канальные / Напольно-потолочные — через новый фильтр `?type=cassette|duct|floor_ceiling`
- Мобильные — без подкатегорий

Новый фильтр `TYPE_CHOICES` в `filters.py` с regex по title (`кассет|канальн|напольн.*потолочн`). `type` сохраняется в hidden input формы фильтров и в кнопке «Сбросить».

**s7. Сортировка/фильтр по цене → `ric` (не `price_wholesale`).**
На B2C-сайте оптовая цена скрыта, юзер видит `ric` (РРЦ). Раньше сортировка «Цена ↑» работала по `price_wholesale` — у части товаров с pw≠ric выглядело «фильтр не работает». Теперь `ordering_map['price']` = `F('ric').asc(nulls_last=True)`, `price_min/price_max` field_name='ric'.

**s8. Share-чипы (product_detail) в эталонном стиле.**
Раньше — dropdown с пунктами Email/TG/MAX/Скопировать. Теперь — 4 цветные pill-кнопки в ряд: **MAX** (фиолетовая outline иконка чата), **Telegram** (голубой самолётик), **Email** (оранжевый конверт), **Ссылка** (серая chain). Эталон — `C:\Users\user\Desktop\клод скрины\сплитхом\поделиться.png`. CSS `.share-chip` + 4 модификатора в `splithub.css`, размер SVG прописан явно (20px desktop / 18px mobile). Первая строка кнопок — Купить в 1 клик / Подобрать другую / Заказать монтаж в одной grid.

**s9. Префилл «+7 » во всех полях телефона.**
Глобальный JS в `base.html`: каждый `<input type="tel">` или `[name="phone"]` с пустым значением получает «+7 » при загрузке (и при `htmx:afterSwap`). Покрывает: регистрация физ/юр, заявка на подбор, монтаж, quick-order, quiz-lead.

**s10. Гео-блок на главной.**
«Монтаж по всему Крыму»: Симферополь / Джанкой / Севастополь / Ялта / Евпатория / Феодосия / Керчь = «в день заказа» (зелёный indicator). «Остальной Крым» = «на следующий день» (приглушённый голубой).

**s11. Django-комментарий артефакт — fix.**
В `_results.html` использовал `{# ... #}` для многострочного комментария — это однострочная форма, текст уехал в HTML как видимая жёлто-зелёная плашка над каталогом. Заменил на `{% comment %}...{% endcomment %}`.

**s12. Race condition деплоя — fix процедуры.**
Скриншот «рассыпался.jpg» — после force-recreate web с `ManifestStaticFilesStorage` HTML сразу ссылается на `splithub.{newhash}.css`, но `rsync` в nginx-path ещё не выполнился (был ПОСЛЕ `up -d`). Окно 1-2с — 404 на CSS → страница без стилей. Новая процедура: `build` → `collectstatic` → **`rsync`** → `up -d --force-recreate`. См. `server_deploy.md`.

### Свежие коммиты (24 мая s-итерация)

```
8ca7d05 fix(home): все названные города — «в день заказа»
d88a32b fix(catalog): сортировка по ric + расширен NON_RETAIL_Q + Джанкой в гео
4b7afc2 feat: префилл «+7 » во всех полях телефона на сайте
9b89212 fix(ui): share-чипы с правильными иконками + мобильный header/каталог
cf9d81e fix: Django-комментарий → {% comment %} + 4 цветные share-чипа
c5203bf feat(catalog): кнопка-toggle «Показать под заказ» прямо в шапке
ca13ec0 feat(ui): контуры кнопок в header + подкатегории в каталоге
492963b fix(catalog): объединить полупромышленные категории + аксессуары
03aeb3e feat(catalog): по умолчанию показываем только то, что в Крыму
4f632ff feat: модальный success для лидов + кнопка «Поделиться»
93c29ea fix(leads): /selection/ — форма пропадает после отправки
```
